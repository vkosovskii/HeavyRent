...
``` bash
docker compose -f deploy/docker-compose.yml restart keycloak
```
...
``` bash
docker compose -f deploy/docker-compose.yml logs keycloak | grep -i "kafka"
```

``` bash
docker compose -f deploy/docker-compose.yml logs keycloak | grep -i "error\|kafka\|producer" | tail -20
```

``` bash
docker compose -f deploy/docker-compose.yml logs keycloak | grep -i "register" | tail -10
```
...
curl -s -o /dev/null -w "%{http_code}"   -X POST 'http://localhost:8080/admin/realms/heavyrent/users'   -H "Authorization: Bearer $TOKEN"   -H 'Content-Type: application/json'   -d '{
...
curl -s 'http://localhost:8080/admin/realms/heavyrent/users'   -H "Authorization: Bearer $TOKEN" | jq .
```
...
``` bash
./gradlew :infrastructure:keycloak-kafka-plugin:jar && docker compose -f deploy/docker-compose.yml restart keycloak && docker compose -f deploy/docker-compose.yml logs -f keycloak | grep -i kafka
```
